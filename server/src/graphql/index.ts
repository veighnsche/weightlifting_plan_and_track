...
import { hasuraWsClient } from "../services/hasura";
...
const resolvers: IExecutableSchemaDefinition["resolvers"] = {
  Query: {
    hello: () => "Hello world!",
    async getWorkouts(_, __, { dataSources }) {
      const response = await dataSources.hasura.queryHasura(gql`
          query GetWorkouts {
              wpt_workouts {
                  workout_id
                  name
...
function startWorkoutsSubscription() {
  return hasuraWsClient().subscribe(
